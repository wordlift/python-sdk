...
async def create_entity_gaps(key: str, configuration: Configuration, account: AccountInfo,
                             url_list: list[str]) -> DataFrame:
...
async def append_entity_gaps_response_to_row_factory(
        entity_gaps_callback: Callable[[Series], Awaitable[Optional[AnalysesResponse]]]) -> Callable[
