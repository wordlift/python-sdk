...
from wordlift_sdk.kg.manager.urlprovider import GoogleSheetsUrlProvider
from wordlift_sdk.kg.manager.urlprovider.url_provider import Url


class TestGoogleSheetsUrlProvider(unittest.TestCase):
    def test_init(self):
        """Test that the GoogleSheetsUrlProvider is initialized correctly."""
        mock_creds = MagicMock()
        url = "https://docs.google.com/spreadsheets/d/example"
        sheet = "Sheet1"
        provider = GoogleSheetsUrlProvider(mock_creds, url, sheet)
    ...
    @patch('wordlift_sdk.kg.manager.urlprovider.google_sheets_url_provider.create_dataframe_from_google_sheets')
    ...
    async def _collect_urls(self, provider: GoogleSheetsUrlProvider) -> List[Url]:
